...
export class CombatScene extends Phaser.Scene {
  ...
  public create(): void {
    ...
    this.drawFigure(width * 0.28, height * 0.54, 0xb7352a, "侠");
    this.drawFigure(width * 0.72, height * 0.5, 0x2f7c6e, "影");

    this.add
      .text(width / 2, height * 0.16, "洛水残照", {
        fontFamily: "serif",
        fontSize: "28px",
        color: "#2c2924"
      })
  ...
  private drawFigure(x: number, y: number, color: number, label: string): void {
    const graphics = this.add.graphics();
